@REM --------------------------------------------------------------------------
@REM Dorset Web Demo Batch Script
@REM 
...
:run
set DORSET_CMD_LINE_ARGS=%*
set DORSET_JAVA_EXE="%JAVA_HOME%\bin\java.exe"

for %%i in (target\web-*.war) do set DORSET_WAR="%%i"

%DORSET_JAVA_EXE% -jar %DORSET_WAR% %DORSET_CMD_LINE_ARGS%
